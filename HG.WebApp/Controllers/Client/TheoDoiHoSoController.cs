...
namespace HG.WebApp.Controllers.Client
{
    public class TheoDoiHoSoController : BaseController
    {
        ...
        public async Task<IActionResult> ExportHSSapHetHan(string from, string to)
        {
            ...
            using (var package = new ExcelPackage(stream))
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    workSheet.Cells[i + 2, 6].Value = item.ten_trang_thai + (item.so_ngay_con == 0 ? " (đến hạn)" : " quá hạn " + item.so_ngay_con + " ngày");
        ...
        [HttpGet]
        public async Task<IActionResult> HSChamTiepNhanPaging(string from, string to, int page = 0)
        {
            var total = 0;
            var pageSize = Convert.ToInt32(_config["AppSetting:PageSize"]);
            var hs_sap_het_han = _hsDao.HoSoSapChamTiepNhan(from, to, page, pageSize, out total);
            ViewBag.TotalPage = (total / pageSize) + ((total % pageSize) > 0 ? 1 : 0);
            ViewBag.CurrentPage = page;
            var result = await CoinExchangeExtensions.RenderViewToStringAsync(this, "~/Views/TheoDoiHoSo/HSChamTiepNhanPaging.cshtml", hs_sap_het_han);
            return Content(result);
        }

        public async Task<IActionResult> ExportHSSapChamTiepNhan(string from, string to)
        {
            // query data from database  
            await Task.Yield();

            var stream = new MemoryStream();
            var list = _hsDao.HoSoSapChamTiepNhannExcel(from, to);
            using (var package = new ExcelPackage(stream))
            {
                var workSheet = package.Workbook.Worksheets.Add("Sheet1");
                var modelTable = workSheet.Cells;
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    workSheet.Cells[i + 2, 4].Value = HG.WebApp.Common.HelperDateTime.DateTimeToYYYYMMDD((DateTime)item.ngay_tiep_nhan);
                    workSheet.Cells[i + 2, 5].Value = HG.WebApp.Common.HelperDateTime.DateTimeToYYYYMMDD((DateTime)item.ngay_hen_tra);
                    workSheet.Cells[i + 2, 6].Value = "Chậm tiếp nhận " + item.so_ngay_con.ToString() + " giờ";
                    workSheet.Cells[i + 2, 7].Value = item.ten_don_vi;
                }
